...
const GlobeExperiment = () => {
  ...
  useEffect(() => {
    ...
    // Create a canvas to draw and sample world map
    const createWorldMapCanvas = () => {
      ...
      ctx.fillStyle = '#000000' // Ocean = black
      ctx.fillRect(0, 0, canvas.width, canvas.height)

      ctx.fillStyle = '#ffffff' // Land = white
      ...
      // Draw simplified continent polygons
      // North America
      ctx.beginPath()
      let points = [
        [72, -168], [72, -140], [70, -130], [60, -125], [50, -128],
        [40, -125], [32, -117], [25, -110], [22, -108], [20, -105],
        [18, -96], [20, -88], [25, -82], [30, -80], [32, -75],
        [40, -74], [45, -68], [50, -65], [60, -65], [65, -70],
        [70, -80], [72, -95], [68, -105], [65, -115], [60, -140],
        [65, -160], [70, -170]
      ]
      points.forEach((p, i) => {
      ...
      ctx.beginPath()
      points = [
        [12, -81], [8, -78], [5, -75], [0, -75], [-5, -75],
        [-10, -78], [-15, -75], [-20, -70], [-25, -70], [-30, -71],
        [-35, -72], [-40, -73], [-45, -72], [-50, -70], [-55, -68],
        [-56, -65], [-54, -60], [-50, -58], [-45, -58], [-40, -60],
        [-35, -58], [-30, -55], [-25, -52], [-20, -48], [-15, -45],
        [-10, -42], [-5, -40], [0, -38], [5, -42], [10, -48],
        [12, -55], [11, -65], [10, -72]
      ]
      points.forEach((p, i) => {
        const { x, y } = toCanvas(p[0], p[1])
      ...
      ctx.beginPath()
      points = [
        [37, -17], [35, -10], [32, -6], [30, 3], [25, 10],
        [20, 15], [15, 20], [10, 25], [5, 30], [0, 35],
        [-5, 38], [-10, 40], [-15, 42], [-20, 45], [-25, 48],
        [-30, 50], [-35, 51], [-34, 45], [-32, 40], [-28, 35],
        [-24, 32], [-20, 30], [-15, 28], [-10, 25], [-5, 20],
        [0, 15], [5, 10], [8, 5], [10, -5], [12, -12],
        [15, -15], [20, -12], [25, -8], [30, -10], [35, -15]
      ]
      points.forEach((p, i) => {
        const { x, y } = toCanvas(p[0], p[1])
        if (i === 0) ctx.moveTo(x, y)
      ...
      ctx.beginPath()
      points = [
        [71, -10], [70, 0], [68, 10], [65, 20], [60, 30],
        [58, 35], [55, 38], [50, 40], [45, 40], [40, 38],
        [38, 30], [40, 20], [45, 15], [50, 10], [55, 5],
        [60, -5], [65, -8], [70, -10]
      ]
      points.forEach((p, i) => {
        const { x, y } = toCanvas(p[0], p[1])
      ...
      ctx.beginPath()
      points = [
        [78, 60], [75, 80], [70, 100], [65, 120], [60, 140],
        [55, 155], [50, 165], [45, 175], [40, 180], [35, 180],
        [30, 175], [25, 165], [20, 150], [15, 135], [10, 120],
        [5, 105], [0, 95], [-5, 90], [-8, 100], [-10, 110],
        [-5, 125], [0, 140], [10, 145], [20, 142], [25, 138],
        [30, 130], [35, 125], [40, 120], [50, 110], [60, 95],
        [70, 80], [75, 70]
      ]
      points.forEach((p, i) => {
        const { x, y } = toCanvas(p[0], p[1])
      ...
      ctx.beginPath()
      points = [
        [-10, 113], [-15, 115], [-20, 118], [-25, 120], [-30, 125],
        [-35, 130], [-38, 138], [-40, 145], [-42, 150], [-43, 153],
        [-42, 150], [-38, 148], [-35, 145], [-30, 142], [-25, 140],
        [-20, 138], [-15, 135], [-12, 125], [-10, 118]
      ]
      points.forEach((p, i) => {
        const { x, y } = toCanvas(p[0], p[1])
        if (i === 0) ctx.moveTo(x, y)
    ...
    // Generate particles only on land
    const generateContinentParticles = () => {
      const points: { lat: number; lon: number }[] = []
      const totalAttempts = 80000 // Attempt many points
    ...
    // Add mobile particles (planes, ships, satellites)
    ...
    const mobileRadii = new Float32Array(mobileCount) // Distance from center

    for (let i = 0; i < mobileCount; i++) {
      const i3 = i * 3

      // Random position on sphere (but varied radius for satellites)
      const isSatellite = Math.random() < 0.15 // 15% are satellites (higher altitude)
      const baseRadius = isSatellite ? radius * 1.3 : radius * 1.05 // Satellites farther out
      ...
      // Random velocity in tangent direction (moving along surface)
      const speed = isSatellite ? 0.4 : 0.25 // Satellites move faster
    ...
    const animate = () => {
      ...
      // Very slow, almost imperceptible globe rotation
      particles.rotation.y += 0.0003
      particles.rotation.x = Math.sin(Date.now() * 0.0001) * 0.05

      // Mouse interaction (keep responsive)
      particles.rotation.y += mouseX * 0.002
      particles.rotation.x += mouseY * 0.002

      // Also rotate mobile particles with globe (but they have independent movement too)
      ...
      // Animate land-based particles (confined to continents)
      ...
      // Animate mobile particles (planes, ships, satellites - free movement)
    ...
    <div className="relative w-full h-screen bg-primary-dark overflow-hidden">
      ...
      {loading && (
        <div className="absolute inset-0 flex items-center justify-center">
          <div className="text-white text-xl">Generating continents...</div>
        </div>
      )}
    </div>
  )
...
export default GlobeExperiment
